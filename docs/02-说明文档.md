...
- 可选 **AI** 辅助：项目中分析/导入任务文本、工作报告与「我的任务」侧计划生成（需管理员配置 OpenRouter）。
...
| 项目工作台 | 看板（拖拽改状态）、列表、甘特图、报表、动态、AI 导入/分析（视配置） |
| 任务 | 标题/描述、状态、优先级、起止日期、进度、负责人、协助人、任务内讨论（与私信独立） |
| 我的任务 | 按状态筛选、月历视图、可选 AI 生成今日/本周计划（OpenRouter） |
| 消息中心 | **通知与提醒**（系统通知）；**私信**（与同组织成员的 1v1，聊天记录持久化） |
| 工作报告 | 视实现与 AI 配置，用于生成或汇总报告类内容 |
...
**Q：AI 功能不可用？**  
A：需部署环境配置 `OPENROUTER_API_KEY`（及可选 `OPENROUTER_MODEL`）；未配置时相关按钮会不可用或提示。
