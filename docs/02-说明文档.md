...
3. 进入项目 → 使用 **看板 / 列表 / 甘特** 等视图管理任务；点任务打开右侧 **任务详情**，编辑名称、状态、负责人、**协助人**（下拉/勾选/已注册邮箱添加）、截止日期等（支持自动保存与失焦保存）。
...
| 组织与项目 | **每人一个工作空间**（一般为首次加入/注册时的组织）；侧栏「项目管理 / 我的项目」汇总当前用户 **ProjectMember** 跨业务组织；新建项目落在个人工作空间；加入项目可通过项目 ID（`POST /api/workspace/join-project`），无需先成为对方组织成员 |
...
**Q：加入项目失败？**  
A：核对项目 ID 是否完整；若使用旧版「组织路径下的加入接口」，仍可能提示「不属于本组织」——请在**唯一工作空间**侧栏「项目管理」中用「加入项目」。侧栏路径统一为 `/org/{主组织}/…`，跨业务项目链接为 `/org/{项目所属 org}/project/…`。
...
**Q：智能任务解析返回「智能解析失败，请稍后重试或简化输入内容」？**  
A：多为网络或服务波动、输入过长；可缩短或分段重试。仍失败请联系管理员排查。

**Q：演示账号（仅当执行过 `npm run db:seed`）？**  
A：以 `DEPLOY.md`、`prisma/seed.ts` 及 `src/lib/demo-account.ts` 中的 `DEMO_EMAIL` / `DEMO_PASSWORD` 为准（当前仓库示例为 `435236356@qq.com`；生产环境请勿依赖默认密码）。
